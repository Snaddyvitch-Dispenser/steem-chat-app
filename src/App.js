...
class SteeMessages extends React.Component {
  constructor(props) {
    ...
    this.state = {'pmAddUsername': 'null', 'showAddChannels': false, 'channelAddName': '', 'showRemoveChannels': false, 'channelListUpdate': 0};
  ...
  removeChannel (name, isPrivateMessage=true) {
    ...
    this.setState({channelListUpdate: this.state.channelListUpdate + 1});
    store.set('channels', this.channel_list);
  }
  ...
  renderChannelList() {
    return (
      <ChannelList showRemoveIcon={this.state.showRemoveChannels} addChannelsDialog={() => this.toggleAddChannels()} removeChannelsDialog={() => this.toggleRemoveChannels()} removeChannel={(name, isPrivateMessage) => this.removeChannel(name, isPrivateMessage)} channels={this.channel_list} />
    );
  }
                ...
                <div className="col col-offset-2">
                  <div>
                    ...
                    <div className="input-with-icon">
                      ...
                      <input type="text" id="private_message_username" onChange={event => this.setState({channelAddName: event.target.value.toLowerCase()})} maxLength="16" />
                      <button className="input-action" onClick={event => {if(this.state.channelAddName !== "") {this.addChannel(this.state.channelAddName,false,"dmessages"); this.setState({showAddChannels: false})}}}>+</button>
                    </div>
                  </div>
                </div>
                <div className="col col-offset-2">
                  <div>
                    ...
                    <div className="input-with-icon">
                      ...
                      <button className="input-action" onClick={event => {if(this.state.pmAddUsername !== "" && this.state.pmAddUsername !== "null") {this.addChannel(this.state.pmAddUsername); this.setState({showAddChannels: false})}}}>+</button>
                    </div>
                  </div>
                </div>
              </div>
...
function ChannelList(props) {
  ...
  return (
    <div id="channel-bar">
      {props.channels.private.map((item) => (<ChannelIcon key={item} showRemoveIcon={props.showRemoveIcon} removeChannel={()=>props.removeChannel(item)} user={item}/>))}
      {first_break}
      {props.channels.group.map((item) => (<ChannelIcon key={item.name} name={item.name} removeChannel={()=>props.removeChannel(item.name, false)} showRemoveIcon={props.showRemoveIcon} avatar={item.avatar}/>))}
      {second_break}
      <div className="command-icon" onClick={props.addChannelsDialog} data-tip="Add">+</div>
      <div className="command-icon" onClick={props.removeChannelsDialog} data-tip="Remove">-</div>
    </div>
  );
}
...
function ChannelIcon(props) {
  ...
  return (
    ...
    style={{backgroundImage: `url( "https://steemitimages.com/u/${avatar_name}/avatar")`}}  alt={channel_name} data-tip={channel_name}><div onClick={props.removeChannel} className="delete-channel">&times;</div></div>
  );
}
